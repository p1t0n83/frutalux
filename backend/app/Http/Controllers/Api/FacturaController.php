...
use App\Models\EntregaSuscripcion;
...
class FacturaController extends Controller
{
    public function generar(Request $request)
    {
        try {
            ...
            // ─────────────────────────────────────────────────────────────
            // SUSCRIPCIÓN → crea Suscripcion + EntregaSuscripcion, NO Pedido
            // ─────────────────────────────────────────────────────────────
            if ($tipoFactura === 'suscripcion') {
                $tipoCaja  = $request->tipo_caja  ?? 'mediana';
                ...
                // Programa las primeras 3 entregas
                for ($i = 1; $i <= 3; $i++) {
                    EntregaSuscripcion::create([
                        'suscripcion_id'   => $suscripcion->id,
                        'fecha_programada' => now()->addDays($diasIntervalo * $i),
                        'estado'           => 'pendiente',
                    ]);
                }

                $textoTipo = '<p style="background:#fff3cd;padding:10px;border-left:4px solid #ffc107;margin:20px 0;">
                    <strong>SUSCRIPCIÓN ACTIVA:</strong> ' . ucfirst($tipoCaja) . ' · ' . ucfirst($frecuencia) . '
                    · Próxima entrega en ' . $diasIntervalo . ' días.</p>';

            // ─────────────────────────────────────────────────────────────
            // PEDIDO normal → crea Pedido como antes
            // ─────────────────────────────────────────────────────────────
            } else {
                ...
                // Guardar factura_url en el pedido al final
                $pedidoParaUrl = $pedido;
            }

            // ─────────────────────────────────────────────────────────────
            // Generar PDF (igual para ambos tipos)
            // ─────────────────────────────────────────────────────────────
            $productos = $request->productos ?? [];
            $subtotal   = number_format($subtotalNumerico, 2, ',', '.');
            $gastosEnvio = number_format($gastosEnvioNumerico, 2, ',', '.');
            $total       = number_format($totalNumerico, 2, ',', '.');
            ...
            $html = $this->generarHtmlFactura(
                $vendedorNombre, $vendedorDireccion, $vendedorCp, $vendedorLocalidad,
                $vendedorPais, $vendedorTelefono, $vendedorCif, $numero, $textoTipo,
                $cliente, $clienteEmail, $clienteDireccion, $clienteCp, $clienteLocalidad,
                $clienteProvincia, $fecha, $productosHtml, $subtotal, $gastosEnvio, $total, $datosPago
            );
            ...
            // Guardar factura_url solo en Pedido (Suscripcion no tiene ese campo)
            if ($tipoFactura !== 'suscripcion' && isset($pedidoParaUrl)) {
                $pedidoParaUrl->update(['factura_url' => "/facturas/{$filename}"]);
            }
...
<head>
    ...
    <style>
        body { font-family: DejaVu Sans, sans-serif; color: #333; font-size: 11px; margin: 0; padding: 0; }
        .container { padding: 40px; max-width: 750px; margin: 0 auto; }
        .header { margin-bottom: 40px; border-bottom: 3px solid #2c3e50; padding-bottom: 20px; overflow: hidden; }
        .header-left { float: left; width: 55%; }
        .header-right { float: right; width: 40%; text-align: right; }
        .company { font-size: 22px; font-weight: bold; color: #2c3e50; margin-bottom: 8px; }
        .company-details { font-size: 9px; color: #7f8c8d; line-height: 1.5; }
        .invoice-title { font-size: 26px; font-weight: bold; color: #2c3e50; margin-bottom: 5px; }
        .invoice-number { color: #7f8c8d; font-size: 11px; }
        .clear { clear: both; }
        .info-section { margin: 30px 0; overflow: hidden; }
        .info-box { background: #f8f9fa; padding: 15px; margin-bottom: 15px; border-left: 4px solid #3498db; float: left; width: 47%; }
        .info-box:last-child { float: right; border-left-color: #e74c3c; }
        .info-box h3 { font-size: 9px; color: #7f8c8d; text-transform: uppercase; margin: 0 0 10px 0; letter-spacing: 1px; }
        .info-box p { margin: 5px 0; font-size: 11px; }
        table { width: 100%; border-collapse: collapse; margin: 30px 0; }
        thead { background: #2c3e50; }
        th { background: #2c3e50; color: #ffffff; padding: 12px 10px; text-align: left; font-weight: 600; font-size: 9px; text-transform: uppercase; letter-spacing: 0.5px; }
        td { padding: 12px 10px; border-bottom: 1px solid #ecf0f1; font-size: 11px; }
        tbody tr:nth-child(even) { background: #f8f9fa; }
        .text-right { text-align: right; }
        .text-center { text-align: center; }
        .totals { margin-top: 30px; float: right; width: 280px; }
        .totals table { margin: 0; }
        .totals td { border: none; padding: 8px 15px; }
        .subtotal-row { font-size: 10px; }
        .total-row { background: #2c3e50; color: #ffffff; font-weight: bold; font-size: 13px; }
        .payment-info { background: #e8f5e9; padding: 15px; margin: 30px 0; border-left: 4px solid #4caf50; clear: both; }
        .payment-info h3 { font-size: 11px; color: #2e7d32; margin: 0 0 10px 0; text-transform: uppercase; }
        .payment-info p { margin: 5px 0; font-size: 10px; }
        .footer { margin-top: 80px; padding-top: 20px; border-top: 2px solid #ecf0f1; text-align: center; font-size: 9px; color: #95a5a6; clear: both; }
